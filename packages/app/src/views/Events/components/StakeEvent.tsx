...
import { AccordionV2, ContentColumns, ContentColumn, Text, Spacer, Button } from '../../../components';
...
const StakeEvent: React.FC<EventsCardProps> = ({
    ...
    return <AccordionV2 title={event.title} isOpen={true} timeLeft={(isClosed() || isComingSoon()) ? (!isComingSoon() ? 'Ended' : 'You have to wait a little bit longer') :  !isClosed() && `${formatTime(
        timeLeft(blocksToSeconds(event.blockEnd - currentBlockNumber)).days,
        timeLeft(blocksToSeconds(event.blockEnd - currentBlockNumber)).hours,
        timeLeft(blocksToSeconds(event.blockEnd - currentBlockNumber)).minutes,
    )} left to enter`}>
        <div>
            <ContentColumns>
                <ContentColumn width="50%">
                    <Text as="p" size='m' lineHeight={1.125}>
                        Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium, non nemo. Omnis aliquam totam dolores sequi, ea optio quibusdam id voluptatem blanditiis ducimus explicabo libero natus molestiae assumenda neque nesciunt.
                    </Text>
            ...
            {event.cardsRequired.map((cardId, index, {length}) => {
                            ...
                            return card && (
                                ...
                                <div style={{position: 'relative', padding: '0 1rem 0 0'}} key={`${event.id}${cardId}${index}`}>
                                    ...
                                    <div style={{position: 'relative'}}>
                                        <img loading='lazy' style={{opacity: isOwned ? 1 : .7}} height='180' src={card.img}/>
                                        {/* <img
                                    ...
                                    <div style={{
                                        ...
                                        {isOwned ?
                                            ...
                                            isItemCard(cardId) ?
                                                ...
                                                :
                                                <ExternalLink href={`${openSeaUri}${getPepemonFactoryAddress(pepemon) ? getPepemonFactoryAddress(pepemon) : '0xCB6768a968440187157Cfe13b67Cac82ef6cc5a4'}/${cardId}/`}> 
                                                    Buy on opensea
                                                </ExternalLink>
                                        }
                                    </div>
                                ...
                                {!(index+1 ===length) && <div style={{width: '100px'}}> <img src={disabledplusIcon} alt='disabledplusIcon' /> </div> }
                                </>
                            )
                        })}
            </StyledRequiredCards>
                ...
                <StyledRequiredCards>
                        {cardsReturned().filter(cardId => cardId !== event.rewardCard).map(((returnedCardId , index, {length}) => {
                            const card = getCardInfo(returnedCardId);
                            return card && (
                                <>
                                <div style={{position: 'relative', padding: '0 1rem 0 0',   flex: '1 1 0px'}} key={`${event.id}${returnedCardId}`}>
                                    <div style={{
                                        fontSize: '15px',
                                        fontWeight: 'bold',
                                        letterSpacing: '1px',
                                 ...
                                 {!(index+1 ===length) && <img src={enabledplusIcon} alt='enabledplusIcon' /> }
                                </>
                            )
                        }))}
                        <div style={{position: 'relative', padding: '0 1rem .5rem 0',flex: '1 1 0px'}} key={`${event.id}${event.rewardCard}`}>
                            ...
                            <img loading='lazy' height='180' style={{marginBottom: '.3rem'}} src={getCardInfo(event.rewardCard).img}/>
                            {/* {!account ? <Button
                                size="sm"
                                variant="secondary"
            ...
            <Spacer size='sm' />
            <ContentColumns>
                {isClosed() ? <Text as="p" size='m' lineHeight={1.125} style={{ margin: "auto" }}>
                    Event has ended
                </Text> : <Button width="25%" styling="purple" style={{ margin: "auto" }}> JOIN EVENT</Button>}
