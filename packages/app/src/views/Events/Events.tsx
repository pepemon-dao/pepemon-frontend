...
import {
  pepechu1stanniversarycard,
  pepemander1stanniversarycard,
  pepertle1stanniversarycard,
  pepesaur1stanniversarycard,
} from "../../assets";
...
export const eventsMeta = {
  ...
  description:
    "Evolve your awesome Pepemon NFT cards to receive exclusive Pepemon NFT rewards",
};
...
const allEventCards = (chainId: number) =>
  EVENTS_PER_CHAIN.has(chainId)
    ? Array.from(
        new Set(
          EVENTS_PER_CHAIN.get(chainId)
            .map((event) =>
              [...event.cardsRequired, event.rewardCard].filter(
                (card) => !(card === 0)
              )
            )
            .flat()
...
const Events: React.FC<any> = () => {
  ...
  const [events, setEvents] = useState(
    EVENTS_PER_CHAIN.has(providerChainId)
      ? EVENTS_PER_CHAIN.get(providerChainId)
      : []
  );

 

  const cardsBalances = useCardsFactoryData(
    allEventCards(providerChainId),
    transactions
  );
  ...
  useEffect(() => {
    pepemon.provider &&
      pepemon.provider.getNetwork().then((network: any) => {
    ...
    <DefaultPage title="Evolve your Pepemons NFTs">
      <AccordionGroup>
        ...
        <AccordionV2 title="My cards" isOpen={true}>
          {cardsOwned && cardsOwned.length > 0 ? (
            cardsOwned.map((el: any) => {
              return (
                <div style={{ margin: "10px" }}>
                  ...
                  >
                    {getCardInfo(el.cardId).title}
                  </div>
                  <div style={{ position: "relative" }}>
                    <img
                      loading="lazy"
                      height="180"
                      src={getCardInfo(el.cardId).img}
                    />
                    <div
                      style={{
                        position: "absolute",
                        ...
                        width: "22px",
                        height: "30px",
                        margin: "32px 20px 2px 98px",
                        padding: "6px 8px",
                        opacity: "0.9",
                        borderRadius: "6px",
                        boxShadow: "1px 2px 3px 0 rgba(121, 121, 121, 0.5)",
            ...
            >
              <div style={{ margin: "10px" }}>
                You have no Pepemon NFTs yet. Grab some so you can start
                evolving them
              </div>
              <ExternalLink
                shadow
            ...
            options={[
              {
                title: "Active Events",
                onClick: () => {
                  setActiveSeries({ title: "Active Events" });
              ...
              },
              {
                title: "Closed Events",
                onClick: () => {
                  setActiveSeries({ title: "Closed Events" });
        ...
        {blockNumber > 0 &&
          events
            .filter((filterEvent) =>
              activeEvents
                ? filterEvent.blockEnd >= blockNumber
                : filterEvent.blockEnd < blockNumber
            )
            .sort((sortEventA, sortEventB) =>
              activeEvents
                ? sortEventA.id - sortEventB.id
                : sortEventB.id - sortEventA.id
            )
            .map((event: any) => {
