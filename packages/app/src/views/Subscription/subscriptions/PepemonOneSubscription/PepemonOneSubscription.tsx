...
const PepemonOneSubscription: React.FC<any> = () => {
	...
	const minLPTokens: BigNumber | null = useLotteryMinLPTokens();
	...
	useEffect(() => {
		const fetchCardMeta = async () => {
			if (rewardCard !== null) {
				const cardMeta = await getCardMeta(
					parseFloat(rewardCard || '0'),
					pepemon
				);
	...
	const stakingDeadline: Date | null = useLotteryStakingDeadline();
	const stakingStart: Date | null = useLotteryStakingStartblock(transaction);
	const hasClaimed = useLotteryHasClaimed(
		rewardCard !== null ? parseFloat(rewardCard) : 0,
		transaction
	);
	const lockedBlocks =
		stakingStart !== null
			? parseFloat(stakingStart) + lockedPeriod - blockNumber
			: 0;

	const canClaimCurrentCard = () => {
		if (stakingDeadline && stakingStart) {
	...
	return (
		<Accordion
			title='Pepemon One Subscription'
			isActive={isStaking !== null && isStaking}>
			<AccordionBodyContent side='left'>
				...
				<ContentColumns>
					<ContentColumn width='50%'>
						...
						<Text
							...
							{parseFloat(
								getBalanceNumber(ppdexUniV2Balance)?.toString() ?? '0'
							).toFixed(2)}
						</Text>
						<Spacer size='md' />
						<Text as='p' lineHeight={1}>
						...
						<Text
							...
							font={theme.font.neometric}>
							{parseFloat(
								getBalanceNumber(stakedBalance)?.toString() ?? '0'
							).toFixed(2)}
						</Text>
					</ContentColumn>
					<ContentColumn width='50%'>
				...
				<div style={{ display: 'inline-flex', flexDirection: 'column' }}>
							...
							<Text as='p' size='s'>
								{minLPTokens
									? `${parseFloat(
											parseFloat(
												(
													(getBalanceNumber(minLPTokens) || 0) + 0.01
												)?.toString()
											).toPrecision(3)
					...
					{allowance && allowance?.comparedTo(minLPTokens || 0) === -1 ? (
						<Button
						...
						<Button
							...
							styling='purple'
							onClick={() =>
								onLotteryStake().then(() => setTransaction(transaction + 1))
							}>
							{isStaking
								? 'Active'
								: minLPTokens &&
