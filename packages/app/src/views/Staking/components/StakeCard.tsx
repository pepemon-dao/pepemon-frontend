...
const StakeCard: React.FC<any> = () => {
	...
	const getPpblzAllowance = useCallback(async () => {
		// @ts-ignore
		let _ppblzAllowance = await contracts.ppblz.allowance(
			account,
			contracts.ppdex.address
		);
		setPpblzAllowance(
			parseFloat(web3.utils.fromWei(_ppblzAllowance.toString()))
		);
	...
	const getUniV2PpblzAllowance = useCallback(async () => {
		// @ts-ignore
		let _uniV2PpblzAllowance = await contracts.uniV2_ppblz.allowance(
			account,
			contracts.ppdex.address
		);
		setUniV2PpblzAllowance(
			parseFloat(web3.utils.fromWei(_uniV2PpblzAllowance.toString()))
		);
	...
	const getUniV2PpblzBalance = useCallback(async () => {
		...
		setUniV2PpblzBalance(
			parseFloat(web3.utils.fromWei(_uniV2PpblzBalance.toString()))
		);
	...
	const getPpblzSupply = useCallback(async () => {
		...
		setTotalPpblzSupply(
			parseFloat(web3.utils.fromWei(_ppblzSupply.toString()))
		);
	...
	const getMyUniV2PpblzStakeAmount = useCallback(async () => {
		...
		setUniV2PpblzStakedAmount(
			parseFloat(web3.utils.fromWei(stakeA.toString()))
		);
	...
	const getPpdexRewards = useCallback(async () => {
		...
		const ppblzStaked = (
			await contracts.ppdex.getAddressPpblzStakeAmount(account)
		).toString();
		const uniV2Staked = (
			await contracts.ppdex.getAddressUniV2StakeAmount(account)
		).toString();
	...
	const stakePpblz = async () => {
		...
		try {
			let stakeRes = await sendTransaction(
				provider,
				async () =>
					await contracts.ppdex.stakePpblz(
	...
	const stakeUniV2Ppblz = async () => {
		...
		try {
			let stakeRes = await sendTransaction(
				provider,
				async () =>
					await contracts.ppdex.stakeUniV2(
	...
	const withdrawPpblz = async () => {
		...
		try {
			let unstakeRes = await sendTransaction(
				provider,
				async () =>
					await contracts.ppdex.withdrawPpblz(
						web3.utils.toWei(ppblzStakeAmount.toString()),
						{ gasLimit: 200000 }
					)
			);
	...
	const withdrawUniV2Ppblz = async () => {
		...
		try {
			let unstakeRes = await sendTransaction(
				provider,
				async () =>
					await contracts.ppdex.withdrawUniV2(
						web3.utils.toWei(uniV2PpblzStakeAmount.toString()),
						{ gasLimit: 200000 }
					)
			);
	...
	const updatePpblzStakingInput = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const cleanedValue = cleanNumberInput(inputValue, 18);
		setPpblzStakeAmount(cleanedValue !== '' ? cleanedValue : null);
	};
				...
				<StakeGridArea>
					...
					<StakeGridAreaBody>
						...
						<div style={{ marginTop: 'auto' }}>
							...
							{(!isApprovedPpblz ||
								ppblzAllowance < parseFloat(ppblzStakeAmount)) && (
								<Button
									styling='purple'
									onClick={approvePpblz}
									{...((isUpdatingRewards || isApprovingPpblz) && {
										disabled: true,
									})}
									width='100%'>
									{isUpdatingRewards
												...
												: ppblzStakeSub &&
												  !ppblzStakeAdd && {
														...
														disabled:
															!(
																parseFloat(ppblzStakeAmount) > 0 &&
																parseFloat(ppblzStakeAmount) <=
																	ppblzStakedAmount
															) || isWithdrawingPpblz,
												  })}>
											{ppblzStakeAdd && !ppblzStakeSub
												? 'Stake'
												: !ppblzStakeAdd && ppblzStakeSub && 'Withdraw'}
										</Button>
									</ContentCentered>
								)}
						...
						<div style={{ marginTop: 'auto' }}>
							...
							{(!isApprovedUniV2Ppblz ||
								...
								<Button
									...
									{...((isUpdatingRewards || isApprovingUniV2Ppblz) && {
										disabled: true,
									})}
									width='100%'>
									{isUpdatingRewards
											...
											{...(uniV2PpblzStakeAdd && !uniV2PpblzStakeSub
												? {
														...
														disabled:
															!(
																parseFloat(uniV2PpblzStakeAmount) > 0 &&
																parseFloat(uniV2PpblzStakeAmount) <=
																	uniV2PpblzBalance
															) || isStakingUniV2Ppblz,
												  }
												: !uniV2PpblzStakeAdd &&
